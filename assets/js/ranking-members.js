...
function renderRanking(){
  ...
  const all = allMembers.map(m=>{
    ...
    return {name:m.name, gender:m.g, bu:buDisplay, isExt:m.isExt, isEx:m.isEx, isDormant:m.isDormant, ...s};
  ...
  if (rankingTbody) {
    rankingTbody.innerHTML=all.map((p,i)=>{
      ...
      return `<tr><td><span class="rank-badge ${bg}">${rank}</span></td>
        <td><strong>${escapeHtml(p.name)}</strong>${exTag}${dormTag}</td><td>${escapeHtml(p.gender)}</td><td>${escapeHtml(String(p.bu))}</td>
